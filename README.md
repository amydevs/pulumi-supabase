...
1. Run the following command to update files to use the name of your provider (third-party: use your GitHub organization/username):
   ...
   - find and replace all instances of the boilerplate `amydevs` with the `ORG` of your provider.
   - replace all instances of the `github.com/amydevs/pulumi-supabase` repository with the `REPOSITORY` location
...
- binary: pulumi-resource-supabase
  ...
  ignore: []
  ldflags:
  - -X github.com/amydevs/pulumi-supabase/provider/pkg/version.Version={{.Tag}}
